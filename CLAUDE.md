...
- `index.html` — the entire site, self-contained (inline CSS + vanilla JS, system fonts,
  no dependencies, no cookies/tracking/keys). Sections: header → men/women/news/about toggle (tab state lives in the URL hash —
  #news etc. are shareable deep links; hashchange drives back/forward) → sticky jump-nav (#jump wrapper is the
  ...
  current tour (scheduled rows carry 📅 GCal/iCal links — iCal points at REAL files under
  ics/, generated by fetch_data.write_ics(); iOS Safari cannot download data: URIs, which
  is how v1 broke on iPhone. ics/india-{men,women}.ics are subscribable full-fixture
  feeds — webcal:// for Apple, calendar.google.com/calendar/r?cid=<url> for Google.
  Durations by format: T20I 4h · ODI 8.5h · Test = Day-1 start + note) → coming up (future series) → results timeline (2025→, grouped by year,
